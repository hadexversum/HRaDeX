...
*Keep in mind that this guide is based on the file types we encountered. As there are many formats and local custom in labeling the data, this article should be treated as a hint how to start, and not finite tutorial. The user needs to be carefull and react accordingly.*
...
Another way is to use HaDeX::read_hdx() function. Be careful, as HaDeX::read_hdx() allows the use of specific files of HDeXaminer origin, but only from the code level, it is not implemented in the application.
...
Calculated kin_dat can be used in following functions: 
